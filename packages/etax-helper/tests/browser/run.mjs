...
try {
 for(const failure of ['none','config','xhr','fetch','both','legacy','future']) {
  ...
  await page.evaluate(()=>new Promise((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open('GET','/once?token=PRIVATE_TOKEN');xhr.onload=resolve;xhr.onerror=reject;xhr.send();}));
  ...
  if(failure==='none') {
    await page.getByRole('button',{name:'固定请求',exact:true}).click();
    await page.getByRole('button',{name:'/once',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:'取消固定',exact:true}).getAttribute('aria-pressed'),'true');
    await page.getByRole('button',{name:'复制 cURL',exact:true}).click();
    assert.match(await page.evaluate(()=>fixture.copied),/curl --disable/);
    assert.doesNotMatch(await page.evaluate(()=>fixture.copied),/PRIVATE_TOKEN/);
    await page.getByRole('button',{name:'返回请求列表',exact:true}).click();
    await page.evaluate(()=>new Promise(resolve=>{const x=new XMLHttpRequest();x.open('GET','/once?q=2');x.onload=resolve;x.send();}));
    await page.getByRole('button',{name:'按路径',exact:true}).click();
    const group=page.getByRole('button',{name:'https://etax.jiangsu.chinatax.gov.cn:8443/once',exact:true});await group.waitFor();
    assert.equal(await group.count(),1);await group.click();
    assert.equal(await page.getByRole('button',{name:'/once',exact:true}).count(),2);
    await page.getByRole('button',{name:'逐条',exact:true}).click();
    await page.evaluate(async()=>{for(let i=0;i<55;i++)await new Promise(resolve=>{const x=new XMLHttpRequest();x.open('GET','/bulk/'+i);x.onload=resolve;x.send();});});
    await page.getByRole('button',{name:'仅固定 · 1 / 10',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:'/once',exact:true}).count(),1);
    await page.getByRole('button',{name:'清空请求',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:'/once',exact:true}).count(),1);
    await page.getByRole('button',{name:'账户',exact:true}).click();await page.getByRole('button',{name:'请求',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:'取消固定请求',exact:true}).count(),1);
    await page.getByRole('button',{name:'清空全部',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:'/once',exact:true}).count(),0);
    await page.evaluate(()=>new Promise(resolve=>{const x=new XMLHttpRequest();x.open('POST','/large');x.onload=resolve;x.send('x'.repeat(70000));}));
    await page.getByRole('button',{name:'/large',exact:true}).click();
    await page.getByText('请求体已截断，仅展示已采集部分。',{exact:true}).waitFor();
    await page.getByRole('button',{name:'响应',exact:true}).click();
    await page.getByText('响应已截断，仅展示已采集部分。',{exact:true}).waitFor();
    await page.getByRole('button',{name:'复制 cURL',exact:true}).click();
    assert.doesNotMatch(await page.evaluate(()=>fixture.copied),/--data-raw/);
    await page.setViewportSize({width:480,height:800});
    assert(await page.locator('.code-view').evaluate(el=>el.clientHeight>30));
    assert(await page.locator('.panel').evaluate(el=>el.scrollWidth<=el.clientWidth));
    await page.setViewportSize({width:1280,height:900});
    await page.getByRole('button',{name:'返回请求列表',exact:true}).click();
    console.log('PASS request tools: pin retention, grouped drilldown, clear semantics, cURL, truncation and narrow layout');
  }
