...
基于 Vue 3、Element Plus 和 vite-plugin-monkey 的浏览器用户脚本。当前版本 **1.2.10**。
...
账户页提供全部账户、我的收藏、最近查看三种视图。收藏项在全部列表中优先显示；点击“查看”显示账户基本信息并记录最近查看，最多保留 20 条。清空最近查看不会删除收藏。当前没有按账户登录的完整接口，所以这里记录的是查看动作，不是登录或登录成功；手动登录不自动关联任何账户。

收藏最多 200 条，使用当前列表中唯一的 Cookie ID 关联，缺失、重复或不安全的数字标识不可收藏，也不计入最近查看；不以企业名称、税号或行号替代。记录只与本次后端返回的账户匹配，服务暂时未返回的记录不会展示，也不自动删除其收藏。

账户偏好单独保存在脚本管理器中，用地区与 API Key 的 SHA-256 摘要隔离不同账户来源，只持久化 Cookie ID、收藏关系和查看时间，不复制 Token、Cookie 内容、API Key、税号或企业名。最多保留 20 个来源，达到上限会拒绝新增来源写入。读取、保存或格式兼容性异常不会覆盖原数据，也不会阻止账户查询和手动登录。跨页面操作会先读取最新偏好再更新；不同页面同时写入仍受脚本管理器无事务能力限制。

搜索支持空格分隔多个关键词，每个关键词都必须在税号、企业名称或 Cookie ID 中命中。切换账户、请求、设置时保留搜索和筛选条件；刷新整个页面或切换 API Key 后重置。搜索文本不写入持久存储。
