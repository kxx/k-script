<template>
  ...
  <template v-if="visible">
    ...
    <aside ref="panel" class="panel" :style="{width: `${width}px`}" role="dialog" aria-modal="true" aria-label="ETax 助手" @keydown="onKeydown">
      ...
      <footer class="panel-footer"><span>{{region ? `当前地区 · ${region}` : '当前页面'}}</span><span class="capture-state"><i :class="{paused}"/>{{paused ? '采集已暂停' : '正在记录请求'}}<span class="count">{{records.length}} / 50</span></span></footer>
    </aside>
  </template>
</template>
<script setup>
